...
public class TabContainerFragment extends Fragment implements RouterProvider, BackButtonListener {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (getChildFragmentManager().findFragmentById(R.id.ftc_container) == null) {
            getCicerone().getRouter().replaceScreen(Screens.forwardScreen(getContainerName(), 0));
        }
    }
    ...
    private Navigator getNavigator() {
        if (navigator == null) {
            navigator = new AppNavigator(getActivity(), R.id.ftc_container, getChildFragmentManager(), getChildFragmentManager().getFragmentFactory());
        }
        return navigator;
    }
